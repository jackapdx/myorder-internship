...
// --- Appendix C ---
#align(center + horizon)[
  #text(size: 20pt)[*ภาคผนวก ค*]
  
  #text(size: 16pt)[เอกสารอื่นๆ]
]
#pagebreak()
...
#par(first-line-indent: 1cm, justify: true)[
  พจนานุกรมข้อมูลนี้แสดงรายละเอียดของตารางและคอลัมน์ที่ใช้ในระบบ MyAcademy ซึ่งเป็นส่วนหนึ่งของการปฏิบัติงานสหกิจศึกษา ข้อมูลต่อไปนี้อธิบายโครงสร้างฐานข้อมูลที่เกี่ยวข้องกับฟังก์ชันการทำงานหลักของระบบ
]
...
#table(
  ...
  inset: 10pt,
  align: horizon,
  table.header([*ชื่อตาราง*], [*คอลัมน์*], [*คำอธิบาย*]),
  [users], [
    - id
    - email
    - name
  ...
  [courses], [
    ...
    - รหัสหลักสูตร (Primary Key)
    - ชื่อหลักสูตร
    - รหัสผู้สอน (Foreign Key)
  ],
  [lessons], [
    - id
    - course_id
    ...
    - รหัสบทเรียน (Primary Key)
    - รหัสหลักสูตร (Foreign Key)
    - ชื่อบทเรียน
    - ลิงก์วิดีโอ
  ]
)
...
#par(first-line-indent: 1cm, justify: true)[
  ตารางข้างต้นแสดงโครงสร้างฐานข้อมูลหลักที่ใช้ในระบบ โดยแต่ละคอลัมน์มีบทบาทสำคัญในการเก็บข้อมูลและเชื่อมโยงกันระหว่างตาราง
]
